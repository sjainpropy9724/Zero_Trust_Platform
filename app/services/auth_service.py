...
from webauthn.helpers.structs import RegistrationCredential, AuthenticationCredential, PublicKeyCredentialDescriptor # noqa
from webauthn.helpers.exceptions import InvalidAuthenticationResponse # noqa
...
def verify_registration(credential: dict, challenge: str, db: Session, username: str):
    ...
    try:
        ...
        if db_user_by_cred_id:
            return None # Credential ID already exists
...
def verify_authentication(
    ...
    try:
        auth_verification = verify_authentication_response(
            ...
            credential_current_sign_count=0, # In prod, you'd store and check this
            require_user_verification=True,
        )
        
        # In a real app, update and store the new sign count
